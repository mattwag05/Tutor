...
# npm ci creates a symlink for the file: protocol dep (pptxgenjs) that
# may dangle on case-sensitive (Linux) Docker builds. Replace with a real copy.
RUN rm -rf node_modules/pptxgenjs && \
...
COPY --from=frontend-builder /app/web/lib/generation/prompts/ ./web/lib/generation/prompts/
