...
def main() -> None:
    ...
    # TUTOR_DATA_DIR anchors the Next.js server's data dir (dataDir()).
    # Without it, `process.cwd()` in dev is web/, so the frontend reads/writes
    # data/user/settings/ under web/ while the backend uses project-root data/
    # — settings saved in the UI then never reach the generation routes
    # (gotcha #43/#46). Pin both processes to the project-root data dir.
    ...
    env_local_path.write_text(
        ...
        f"DEEPTUTOR_API_URL={api_base}\n"
        f"TUTOR_DATA_DIR={data_dir}\n",
        encoding="utf-8",
    )
    ...
    frontend_env["TUTOR_DATA_DIR"] = data_dir
