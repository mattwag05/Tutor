...
def test_resolve_adapter_class_rejects_openrouter_no_embeddings_api() -> None:
    # OpenRouter is recognized for host/model auto-detection but has no
    # embeddings API; resolving it raises an actionable message (DeepTutor-0xn).
    with pytest.raises(ValueError, match="OpenRouter has no embeddings API"):
        _resolve_adapter_class("openrouter")
...
def test_embedding_client_rejects_openrouter_no_embeddings_api() -> None:
    # The unsupported-binding error fires before any endpoint-format check, so
    # the user sees "no embeddings API" rather than a URL-shape complaint.
    cfg = EmbeddingConfig(
...
def test_every_registered_provider_has_adapter() -> None:
    """Every usable EMBEDDING_PROVIDERS entry must resolve to a valid adapter.

    Providers flagged ``unsupported_reason`` (e.g. openrouter — no embeddings
    API) are recognized for auto-detection but intentionally raise on resolve.
    """
