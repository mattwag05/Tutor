"""Tests for the runtime config adapter."""
...
def test_search_empty_provider_defaults_to_duckduckgo(tmp_path: Path) -> None:
    catalog = _build_catalog(
        search_profile={
            "id": "search-p",
            "name": "Search",
    ...
    resolved = resolve_search_runtime_config(catalog=catalog, env_store=_empty_env(tmp_path))
    assert resolved.provider == "duckduckgo"
    assert resolved.requested_provider == "duckduckgo"
