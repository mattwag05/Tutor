...
def test_ui_saved_default_profile_is_not_overlaid_by_env(
    isolated_stores: tuple[Path, Path],
) -> None:
    """The common bug the user hit: edit the single default profile in the UI
    (model + api_key), click Save — and on the next load the catalog still has
    one default-id profile, so it looks 'pristine' and the env overlay used to
    clobber the just-saved values. A UI save stamps ``user_managed`` on the
    catalog; once set, the env overlay must be skipped so the user's values win
    even though the profile count/id is unchanged."""
    catalog_path, env_path = isolated_stores

    # .env still holds the previous (stale) values.
    _write_env(
        env_path,
        [
            "EMBEDDING_BINDING=openai",
            "EMBEDDING_MODEL=stale-model",
            "EMBEDDING_HOST=https://stale.example/v1/embeddings",
            "EMBEDDING_API_KEY=sk-stale",
        ],
    )

    # What a UI Save Draft persists: the user edited the default profile in
    # place and the save path stamped user_managed=True.
    ui_saved_catalog: dict[str, Any] = {
        "user_managed": True,
        "services": {
            "llm": {"active_profile_id": None, "profiles": []},
            "embedding": {
    ...
    catalog_path.write_text(json.dumps(ui_saved_catalog))

    loaded = ModelCatalogService(catalog_path).load()

    profile = loaded["services"]["embedding"]["profiles"][0]
    # The user's typed values must survive — not be overlaid by stale .env.
    assert profile["api_key"] == "sk-user-typed"
    assert profile["base_url"] == "https://api.openai.com/v1/embeddings"
    assert profile["models"][0]["model"] == "text-embedding-3-large"
    # The flag must persist across the load→normalize→save round-trip.
    saved = json.loads(catalog_path.read_text(encoding="utf-8"))
    assert saved.get("user_managed") is True


def test_save_with_user_managed_stamps_and_persists_the_flag(
    isolated_stores: tuple[Path, Path],
) -> None:
    """The UI save path calls ``save(catalog, user_managed=True)``; the flag is
    written to disk so subsequent loads treat the catalog as authoritative."""
                    ...
                    "profiles": [
                        {
                            ...
                            "api_key": "sk-user-typed",
                            "models": [
                                {"id": "embedding-model-default", "name": "m", "model": "x"}
                            ],
                        }
                    ],
                },
