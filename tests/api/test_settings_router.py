...
from deeptutor.services.config.response_sanitizer import sanitize_catalog_for_response
...
class _FakeCatalogService:
    ...
    def save(self, catalog: dict[str, Any], user_managed: bool = False) -> dict[str, Any]:
        self._catalog = deepcopy(catalog)
        if user_managed:
            self._catalog["user_managed"] = True
    ...
    def apply(
        ...
        current = self.save(catalog if catalog is not None else self.load(), user_managed=user_managed)
...
async def test_update_catalog_invalidates_runtime_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # A UI save stamps user_managed=True so the env overlay can't later revert it.
    assert response == {
        "catalog": sanitize_catalog_for_response({**updated_catalog, "user_managed": True})
    }
    assert service.load()["services"]["llm"]["profiles"][0]["api_key"] == "new-llm-key"
...
async def test_apply_catalog_invalidates_runtime_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert response["catalog"] == sanitize_catalog_for_response(
        {**applied_catalog, "user_managed": True}
    )
    assert response["env"]["LLM_MODEL"] == "gpt-after-apply"
    assert response["env"]["LLM_API_KEY"] == "afte...-key"
    assert response["env"]["EMBEDDING_MODEL"] == "text-embedding-after-apply"
    assert response["env"]["EMBEDDING_API_KEY"] == "afte...-key"
