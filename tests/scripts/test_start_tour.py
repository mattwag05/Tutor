...
class TestResolvePython:
    ...
    def test_install_commands_support_web_profiles(self) -> None:
        start_tour = _load_start_tour_module()
        catalog: dict = {"services": {}}

        web_cmds = start_tour._install_commands("web-basic", catalog)

        assert any("requirements/server.txt" in cmd for cmd, _cwd in web_cmds)
        assert any(
            cmd[0].startswith("npm") or cmd[0].endswith("npm") for cmd, _cwd in web_cmds
        )
