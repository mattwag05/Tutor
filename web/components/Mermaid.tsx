...
export const Mermaid: React.FC<MermaidProps> = ({ chart, className = "" }) => {
  ...
  const [stableChart, setStableChart] = useState(chart);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (lastChartRef.current === chart) setStableChart(chart);
    }, DEBOUNCE_MS);
  ...
  useEffect(() => {
    if (stableChart !== chart) return;

    let cancelled = false;
    const renderChart = async () => {
      if (!stableChart.trim() || !containerRef.current) return;
