...
export default function CreateKbModal({
  ...
  // Surface why the disabled Create button can't be clicked — without this the
  // dialog looks broken when no file is attached (DeepTutor-rnr).
  const blockReason = submitting
    ? null
    : trimmed.length === 0
      ? t("Enter a name to continue")
      : selection.invalidFiles.length > 0
        ? t("Remove the invalid files to continue")
        : selection.validFiles.length === 0
          ? t("Attach at least one document to create a knowledge base")
          : null;
