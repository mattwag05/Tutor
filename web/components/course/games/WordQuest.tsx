...
export function WordQuest({ course }: Props) {
  ...
  if (glossary.length === 0) {
    return (
      <div className="flex flex-col items-center gap-4 py-16 text-neutral-500">
        <p className="text-lg">{t('course.wordQuestEmptyTitle')}</p>
        <p className="text-sm">{t('course.wordQuestEmptyHint')}</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center gap-6 py-16">
        ...
        <h2 className="text-2xl font-bold text-neutral-900 dark:text-neutral-50">
          {t('course.wordQuestComplete')}
        </h2>
        <p className="text-lg text-neutral-600 dark:text-neutral-400">
          {t('course.wordQuestMatchedCount', { score, total: glossary.length })}
        </p>
        <button
          type="button"
          onClick={reset}
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-4">
      ...
      <p className="text-sm text-neutral-500">{t('course.wordQuestInstructions')}</p>
