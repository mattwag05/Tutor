...
export function FillBlankQuizBlockView({ block, onAttempt }: Props) {
  ...
  const onChoose = (letter: string) => {
    ...
    setRevealed(true);
    onAttempt?.({
      isCorrect: letter === block.correctAnswer,
      userAnswer: letter,
    });
  };
        ...
        {parts.map((part, i) => (
          <span key={i}>
            ...
            {i < parts.length - 1 && (
              <span
                aria-hidden="true"
                className="mx-1 inline-block min-w-20 border-b-2 border-neutral-400 align-baseline"
              />
            )}
          </span>
        ))}
