...
function readStoredCollapsed(date: string): boolean {
  try {
    return window.localStorage.getItem(collapsedKey(date)) === "1";
  } catch {
    return false;
  }
}
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (cancelled) return;
        setResponse(next);
        if (next.date) setCollapsed(readStoredCollapsed(next.date));
