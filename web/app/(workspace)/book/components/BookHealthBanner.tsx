...
export default function BookHealthBanner({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setHealth({
          bookId,
          kbDrift: data.kb_drift,
          logHealth: data.log_health,
        });
      } catch {
  ...
  const dismissed =
    dismissedState.bookId === bookId ? dismissedState.dismissed : false;
  if (!bookId || dismissed) return null;

  const kbDrift = health?.bookId === bookId ? health.kbDrift : null;
  const logHealth = health?.bookId === bookId ? health.logHealth : null;
  ...
  const acknowledge = async () => {
    ...
    try {
      ...
      setHealth((current) => ({
        bookId,
        kbDrift: { has_drift: false },
        logHealth: current?.bookId === bookId ? current.logHealth : null,
      }));
    } finally {
      setBusy(false);
    }
    ...
    <div className="mx-6 mt-4 rounded-xl border border-amber-300/60 bg-amber-50 px-4 py-3 text-sm text-amber-900 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
      <div className="flex items-start gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={() => setDismissedState({ bookId, dismissed: true })}
            aria-label={t("Close")}
