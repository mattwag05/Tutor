...
export default function PageReader({
  ...
  const headerCollapsed =
    headerState.pageId === pageId ? headerState.collapsed : false;
  const userToggled =
    headerState.pageId === pageId ? headerState.userToggled : false;
  const setHeaderForPage = useCallback(
    (next: Pick<HeaderState, "collapsed" | "userToggled">) => {
      setHeaderState({ pageId, ...next });
    },
    [pageId],
  );
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      if (top <= 8) {
        setHeaderForPage({ collapsed: false, userToggled: false });
        return;
      }
      if (userToggled) return;
      // Collapse on downward scroll past a small threshold.
      if (top > last && top > 80) {
        setHeaderForPage({ collapsed: true, userToggled: false });
      }
    };
    scrollContainer.addEventListener("scroll", handler, { passive: true });
    return () => scrollContainer.removeEventListener("scroll", handler);
  }, [scrollContainer, setHeaderForPage, userToggled]);
            ...
            <button
              ...
              onClick={() => {
                setHeaderState((current) => ({
                  pageId,
                  collapsed:
                    current.pageId === pageId ? !current.collapsed : true,
                  userToggled: true,
                }));
              }}
