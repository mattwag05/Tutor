...
import { exportBookPdf } from "@/lib/book-export";
...
function BookPageInner() {
  ...
  useEffect(() => {
    let cancelled = false;
    queueMicrotask(() => {
      if (!cancelled) void refreshBooks();
    });
    return () => {
      cancelled = true;
    };
  ...
  const handleExportPdf = async () => {
    ...
    try {
      await exportBookPdf(detail.book.id, detail.book.title);
    } catch (err) {
      console.error("Book PDF export failed", err);
      alert(t("Could not export this book to PDF. Please try again."));
    } finally {
    ...
    <div className="flex h-dvh w-full">
      {view !== "list" && (
        <BookSidebar
          ...
          onExportPdf={detail ? () => void handleExportPdf() : undefined}
