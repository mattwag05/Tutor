...
function mergeCatalogPreservingSecrets(current: Catalog, incoming: Catalog): Catalog {
  const merged = cloneCatalog(incoming);
  const serviceNames = Object.keys(merged.services) as Array<keyof Catalog["services"]>;

  for (const serviceName of serviceNames) {
    const incomingProfiles = merged.services[serviceName].profiles;
    const currentProfiles = current.services[serviceName]?.profiles ?? [];

    incomingProfiles.forEach((incomingProfile, index) => {
      const currentProfile =
        currentProfiles.find((profile) => profile.id === incomingProfile.id) ??
        currentProfiles[index];
      const currentKey = currentProfile?.api_key;
      if (
        incomingProfile.api_key &&
        isMaskedSecret(incomingProfile.api_key) &&
        currentKey &&
        !isMaskedSecret(currentKey)
      ) {
        incomingProfile.api_key = currentKey;
      }
    });
  }
...
function providerPresetLimit(service: ServiceName): number {
  if (service === "manifest") return 0;
  if (service === "embedding") return 3;
  if (service === "search") return 4;
  return 3;
}

function isServiceRunnable(
  service: ServiceName,
  profile: CatalogProfile | null,
  model: CatalogModel | null,
): boolean {
  if (service === "manifest") return true;
  if (!profile) return false;
  if (service === "search") return Boolean(profile.provider);
  return Boolean(model?.model && (profile.binding || profile.base_url));
}
...
function SettingsPageContent() {
  ...
  useEffect(() => {
    const load = async () => {
      const [settingsResponse, statusResponse, manifestResponse, agentResponse] =
        await Promise.all([
  ...
  const activeRunnable = isServiceRunnable(activeService, activeProfile, activeModel);
  const controlsBusy = saving || applying || testRunning !== null || savingManifest;
  ...
  const persistUi = async (patch: Partial<UiSettings>) => {
    try {
      ...
      setToast(`${t("Save failed")}: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
  ...
  const addProfile = () => {
    ...
    mutateCatalog((next) => {
      ...
      const profile: CatalogProfile = {
        ...
        provider: isSearch ? "duckduckgo" : undefined,
        base_url: "",
        api_key: "",
        api_version: "",
        extra_headers: isSearch ? undefined : {},
        proxy: isSearch ? "" : undefined,
        request_timeout: svc === "embedding" ? "60" : undefined,
        batch_size: svc === "embedding" ? "10" : undefined,
        batch_delay: svc === "embedding" ? "0" : undefined,
        models: [],
      };
      if (!isSearch) {
  ...
  const updateProfileNumberField = (
    field: keyof CatalogProfile,
    value: string,
    fallback: string,
  ) => {
    updateProfileField(field, value.replace(/[^\d.]/g, "") || fallback);
  };
  ...
  // Parse a settings response and throw a useful message on failure, so a
  // failed save surfaces an error toast instead of silently showing "saved".
  const readSettingsResponse = async (response: Response) => {
    let payload: { catalog?: Catalog; detail?: string; error?: string } | null = null;
    try {
    ...
    if (!response.ok || !payload?.catalog) {
      throw new Error(
        payload?.detail || payload?.error || `${response.status} ${response.statusText}`,
      );
    }
    return payload as { catalog: Catalog };
  };
  ...
  const saveCatalog = async () => {
    ...
    try {
      ...
      const mergedCatalog = mergeCatalogPreservingSecrets(draft, payload.catalog);
      setCatalog(mergedCatalog);
      setDraft(cloneCatalog(mergedCatalog));
  ...
  const saveManifest = async () => {
    ...
    try {
      ...
      if (!manifestRes.ok || !agentRes.ok) {
        const which = !manifestRes.ok ? manifestRes : agentRes;
        throw new Error(`${which.status} ${which.statusText}`);
      }
      ...
      source.onmessage = (event) => {
        ...
        if (entry.catalog) {
          setCatalog((currentCatalog) =>
            mergeCatalogPreservingSecrets(currentCatalog, entry.catalog!),
          );
          setDraft((currentDraft) =>
            mergeCatalogPreservingSecrets(currentDraft, entry.catalog!),
          );
        }
      ...
      <div className="mx-auto max-w-[960px] px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            ...
            <button
              ...
              disabled={controlsBusy || !hasUnsavedChanges}
        ...
        <div className="mb-8">
            ...
            <div className="flex items-center gap-2">
              {activeService === "manifest" ? (
                <button
                  ...
                  disabled={controlsBusy || !manifestDirty}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border)]/50 px-3 py-1.5 text-[12px] font-medium text-[var(--muted-foreground)] transition-colors hover:border-[var(--border)] hover:text-[var(--foreground)] disabled:opacity-40"
                >
                      ...
                      <div className="grid grid-cols-[1fr_120px] gap-3 sm:grid-cols-[2fr_1fr_1fr_1fr]">
                        <div>
                          ...
                          <input
                            ...
                            placeholder={t("settings.manifestModelPlaceholder")}
                            className={inputClass}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            placeholder={t("settings.bindingPlaceholder")}
                            className={inputClass}
                          />
                        </div>
                        <div>
          ...
          {activeService !== "manifest" && (activeProfile ? (
            <>
              <div className="mb-5 rounded-xl border border-[var(--border)]/70 bg-[var(--muted)]/25 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span
                        className={`h-2 w-2 rounded-full ${activeRunnable ? "bg-emerald-500" : "bg-amber-500"}`}
                      />
                      <h2 className="truncate text-[14px] font-semibold text-[var(--foreground)]">
                        {activeProfile.name}
                      </h2>
                      {servicesPendingApply[activeService] && (
                    ...
                    <p className="mt-1 text-[12px] text-[var(--muted-foreground)]">
                      {activeRunnable
                        ? t("Ready to test and apply.")
                        : t("Complete provider, endpoint, and model details before testing.")}
                    </p>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    <span className="rounded-lg border border-[var(--border)]/70 bg-[var(--card)] px-2.5 py-1 text-[11px] text-[var(--muted-foreground)]">
                    ...
                    {controlsBusy && (
                      <span className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border)]/70 bg-[var(--card)] px-2.5 py-1 text-[11px] text-[var(--muted-foreground)]">
                        <Loader2 className="h-3 w-3 animate-spin" />
                        {t("Running")}
                      </span>
                    )}
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <div className="min-w-0 rounded-lg bg-[var(--card)]/70 p-3">
                    <div className={`${labelClass("sm")} text-[var(--muted-foreground)]`}>
                      {t("Provider")}
                    </div>
                    <div className="mt-1 truncate text-[13px] font-medium text-[var(--foreground)]">
                      {activeService === "search"
                        ? activeProfile.provider || t("No provider")
                        : activeProfile.binding || t("No provider")}
                    </div>
                  </div>
                  <div className="min-w-0 rounded-lg bg-[var(--card)]/70 p-3">
                    <div className={`${labelClass("sm")} text-[var(--muted-foreground)]`}>
                      {activeService === "embedding" ? t("Endpoint URL") : t("Endpoint")}
                    </div>
                    <div className="mt-1 truncate text-[13px] font-medium text-[var(--foreground)]">
                      {activeProfile.base_url || t("Not set")}
                    </div>
                  </div>
                  <div className="min-w-0 rounded-lg bg-[var(--card)]/70 p-3">
                    <div className={`${labelClass("sm")} text-[var(--muted-foreground)]`}>
                      {activeService === "search" ? t("Results") : t("Model")}
                    </div>
                    <div className="mt-1 truncate text-[13px] font-medium text-[var(--foreground)]">
                      {activeService === "search"
                        ? t("{{count}} max", { count: activeProfile.max_results || 5 })
                        : activeModel?.model || t("No model selected")}
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {(providers[activeService] || [])
                          .slice(0, providerPresetLimit(activeService))
                          .map((preset) => {
                            const selected =
                              activeService === "search"
                                ? activeProfile.provider === preset.value
                                : activeProfile.binding === preset.value;
                            return (
                              <button
                                key={preset.value}
                                type="button"
                                onClick={() => {
                                  const field =
                                    activeService === "search"
                                      ? "provider"
                                      : "binding";
                                  updateProfileField(field, preset.value);
                                  if (preset.base_url) {
                                    updateProfileField("base_url", preset.base_url);
                                  }
                                  if (
                                    activeService === "embedding" &&
                                    preset.default_dim
                                  ) {
                                    updateModelField("dimension", preset.default_dim);
                                  }
                                }}
                    ...
                    {activeService === "search" ? (
                      <>
                        <div>
                          ...
                          <input
                            className={inputClass}
                            value={String(activeProfile.max_results || 5)}
                            inputMode="numeric"
                            onChange={(e) =>
                              updateProfileNumberField(
                                "max_results",
                                e.target.value,
                                "5",
                              )
                          ...
                          />
                          <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
                            {t("Clamped to 1-10 by the search runtime.")}
                          </p>
                        </div>
                        <div>
                          <div className="mb-1.5 text-[12px] text-[var(--muted-foreground)]">
                    ...
                    {activeService === "embedding" && (
                      <div className="grid gap-4 sm:col-span-2 sm:grid-cols-3">
                        <div>
                          ...
                          <input
                            className={inputClass}
                            value={activeProfile.request_timeout || "60"}
                            inputMode="numeric"
                            onChange={(e) =>
                              updateProfileNumberField(
                                "request_timeout",
                        ...
                        <div>
                          ...
                          <input
                            className={inputClass}
                            value={activeProfile.batch_size || "10"}
                            inputMode="numeric"
                            onChange={(e) =>
                              updateProfileNumberField(
                                "batch_size",
                        ...
                        <div>
                          ...
                          <input
                            className={inputClass}
                            value={activeProfile.batch_delay || "0"}
                            inputMode="decimal"
                            onChange={(e) =>
                              updateProfileNumberField(
                                "batch_delay",
                        ...
                        <p className="sm:col-span-3 text-[11px] leading-relaxed text-[var(--muted-foreground)]">
                          {t(
                            "Timeout is seconds per request; batch controls tune embedding throughput for slower local or hosted endpoints.",
                          )}
                        </p>
                      </div>
                    )}
          ...
          <div className="flex items-center justify-between px-5 py-3.5">
            ...
            <div className="ml-3 flex items-center gap-3">
              <button
                ...
                disabled={controlsBusy || !activeRunnable}
                title={
                  activeRunnable
                    ? t("Run test")
                    : t("Complete the active provider and model first")
                }
              ...
              >
                ...
                {testRunning === activeService ? t("Running") : t("Run test")}
              </button>
              <button
                type="button"
                onClick={() => setDiagnosticsOpen((v) => !v)}
