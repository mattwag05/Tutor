...
test.describe("Tutor course flow", () => {
  ...
  test("creator renders and selects every v1 format", async ({ page }) => {
    ...
    await expect(
      page.evaluate(() => {
        const settings = document.querySelector("#course-advanced-settings");
        const formatStep = Array.from(document.querySelectorAll("div")).find((node) =>
          node.textContent?.trim().startsWith("2. Choose formats"),
        );
        return Boolean(
          settings &&
            formatStep &&
            (settings.compareDocumentPosition(formatStep) & Node.DOCUMENT_POSITION_FOLLOWING),
        );
      }),
    ).resolves.toBe(true);
    await page.getByRole("button", { name: "Advanced settings", exact: true }).click();
    await expect(page.getByText("Primary focus")).toHaveCount(0);
    ...
    await fileChooser.setFiles([
      {
        ...
        buffer: Buffer.from("# Learning notes"),
      },
      {
        name: "diagram-brief.txt",
        mimeType: "text/plain",
        buffer: Buffer.from("Explain the diagram in plain language."),
      },
    ]);
    const attachedSources = page.getByRole("list", { name: "Attached source files" });
    await expect(attachedSources.getByText("learning-notes.md")).toBeVisible();
    await expect(attachedSources.getByText("diagram-brief.txt")).toBeVisible();
    await expect(page.locator("form").getByText("2 files")).toBeVisible();

    await page.getByRole("button", { name: "Remove learning-notes.md" }).click();
    await expect(attachedSources.getByText("learning-notes.md")).toHaveCount(0);
    await expect(attachedSources.getByText("diagram-brief.txt")).toBeVisible();
    await expect(page.getByRole("button", { name: "Remove diagram-brief.txt" })).toBeVisible();
  ...
  test("reader progress, drawers, sources, formats, checks, and follow-ups work", async ({
    ...
    try {
      ...
      await expect(
        page.getByRole("progressbar", { name: "Reading progress for Why Overthinking Feels Useful" }),
      ).toHaveAttribute("aria-valuenow", "0");
      await expect(page.getByRole("button", { name: "Advance" })).toHaveCount(0);
      await expect(page.getByLabel("Ask Tutor a question")).toBeVisible();
      await expect(page.getByLabel("Go deeper custom prompt")).toBeVisible();
      ...
      await expect(
        tocDialog.getByText("Navigate 2 sections and course formats for Decision Making Without Overthinking."),
      ).toBeVisible();
      ...
      await expect(page.getByRole("dialog", { name: "Course contents" })).toBeVisible();
      await tocDialog
        .getByRole("button", { name: "Open section 1: Why Overthinking Feels Useful", exact: true })
        .click();
      await expect(page.getByRole("dialog", { name: "Course contents" })).toHaveCount(0);
      await expect(page.locator("body")).not.toContainText("Podcast");
      await page.getByRole("button", { name: "Open table of contents" }).click();
      await page.getByRole("dialog", { name: "Course contents" })
        ...
        .click();
      await expect(page.getByRole("dialog", { name: "Course contents" })).toHaveCount(0);
      await expect(page.getByRole("dialog", { name: "Podcast" })).toBeVisible();
      await page.keyboard.press("Escape");
      await expect(page.getByRole("dialog", { name: "Podcast" })).toHaveCount(0);
      ...
      await expect(
        sourcesDialog.getByText("2 source-backed references are available for this course."),
      ).toBeVisible();
      ...
      await expect(page.getByText("Add a follow-up lesson section.")).toBeVisible();
      await expect(page.getByText("Create an audio walkthrough.")).toBeVisible();
      await expect(page.getByRole("button", { name: "Attach supporting files" })).toHaveCount(0);
      await expect(page.getByRole("dialog", { name: "Add format" })).toBeVisible();
      await page.keyboard.press("Escape");
      await expect(page.getByRole("dialog", { name: "Add format" })).toHaveCount(0);

      await page
        .getByRole("button", { name: "Choose answer B: To prevent analysis from turning into rumination" })
        .click();
      ...
      await expect
        .poll(async () =>
          Number(
            await page
              .getByRole("progressbar", { name: "Reading progress for Why Overthinking Feels Useful" })
      ...
      await page.getByLabel("Ask Tutor a question").fill("Explain with a workplace example");
  ...
  test("reader shows recoverable section and artifact failures", async ({ page, request }) => {
    ...
    try {
      ...
      await page.getByRole("button", { name: "Open table of contents" }).click();
      await page.getByRole("dialog", { name: "Course contents" })
        .getByRole("button", { name: "Open section 2: Build Better Defaults", exact: true })
        .click();
      ...
      const diagramDialog = page.getByRole("dialog", { name: "Diagram" });
      await expect(diagramDialog).toBeVisible();
      await expect(diagramDialog.getByText("Generation failed")).toBeVisible();
      await expect(diagramDialog.getByText("Diagram service unavailable")).toBeVisible();
      await diagramDialog.getByRole("button", { name: "Close" }).click();
      await expect(page.getByRole("dialog", { name: "Diagram" })).toHaveCount(0);
