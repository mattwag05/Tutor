...
function isNonTranslatableText(text) {
  return NON_TRANSLATABLE_TEXTS.has(text);
}

function isCodeFragment(text) {
  if (text === "Promise") return true;
  if (/\b[A-Za-z_][\w.]*\s*\|\s*[A-Za-z_][\w.]*/.test(text)) return true;
  if (text.includes("VariantProps")) return true;
  if (text.includes("InternalNode")) return true;
  if (text.includes("]:") || text.includes('">')) return true;
  if (text.includes(". Next.js")) return true;
  return false;
}
...
function auditFile(content) {
  ...
  for (const m of content.matchAll(jsxTextRe)) {
    ...
    if (isCodeFragment(text)) continue;
