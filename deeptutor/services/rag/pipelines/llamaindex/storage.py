...
def _storage_path_from_version_meta(version: dict[str, Any] | None) -> Path | None:
    # index_versioning's entry builders (_entry_from_flat_version /
    # _entry_from_legacy_nested / _entry_from_root_legacy) always set
    # storage_path for every layout, so trust that invariant rather than
    # re-deriving it from version_path/layout here.
    if not version:
        return None
    storage_path = version.get("storage_path")
    return Path(str(storage_path)) if storage_path else None
...
def resolve_add_storage_plan(kb_dir: Path, signature: EmbeddingSignature | None) -> AddStoragePlan:
    ...
    existing_storage = _storage_path_from_version_meta(matching_version)
    if matching_version and existing_storage is None:
