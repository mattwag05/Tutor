...
class SQLiteSessionStore(AsyncSQLiteStore):
    ...
    DeepTutor-khi resolution: keep the lock (option b). Option (a) —
    making subscribe_turn await write completion explicitly — was
    considered and rejected: it would restructure the hot WS read-after-
    write streaming path for no benefit on a single-user install, whereas
    the lock gives the ordering guarantee for free. The opt-out
    (serialize=False) stays available for stores with no such dependency.
    """
