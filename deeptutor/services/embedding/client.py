...
def _resolve_adapter_class(binding: str) -> type[BaseEmbeddingAdapter]:
    ...
    if spec.unsupported_reason:
        raise ValueError(
            f"Embedding binding '{binding}' is not usable: {spec.unsupported_reason}"
        )
...
class EmbeddingClient:
    ...
    def __init__(self, config: Optional[EmbeddingConfig] = None):
        ...
        # Resolve the adapter first so an unsupported binding (e.g. openrouter,
        # which has no embeddings API) raises its actionable message before the
        # endpoint-format check can complain about the URL instead.
