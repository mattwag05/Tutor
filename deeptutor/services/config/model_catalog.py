...
class ModelCatalogService:
    ...
    def load(self) -> dict[str, Any]:
        if self.path.exists():
            ...
            # Once the user has saved the catalog through the UI we stamp
            # ``user_managed`` and the catalog becomes the sole source of
            # truth — even for an unchanged single default profile. Without
            # this, editing the default profile in place (the most common
            # flow) leaves the catalog looking "pristine" and the env overlay
            # below would silently revert the user's saved model / api_key.
            synced = False
            if not catalog.get("user_managed") and self._is_catalog_pristine(catalog):
    ...
    def save(self, catalog: dict[str, Any], user_managed: bool = False) -> dict[str, Any]:
        normalized = deepcopy(catalog)
        # ``user_managed`` is sticky: an explicit UI save sets it; internal
        # saves (load()-time normalization) pass user_managed=False and
        # preserve whatever the catalog already carries.
        if user_managed:
            normalized["user_managed"] = True
    ...
    def apply(
        self, catalog: dict[str, Any] | None = None, user_managed: bool = False
    ) -> dict[str, str]:
        current = self.save(catalog or self.load(), user_managed=user_managed)
            ...
            for profile in profiles:
                ...
                if service_name == "search":
                    ...
                    if service_name == "embedding":
                        profile.setdefault("request_timeout", "60")
                        profile.setdefault("batch_size", "10")
                        profile.setdefault("batch_delay", "0")
