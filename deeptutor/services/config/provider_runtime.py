...
class EmbeddingProviderSpec:
    ...
    # Set when a provider is recognized (for host/model auto-detection) but has
    # NO usable embeddings endpoint. Resolving it raises this message early
    # instead of failing opaquely at the first HTTP call.
...
EMBEDDING_PROVIDERS: dict[str, EmbeddingProviderSpec] = {
    ...
    "openrouter": EmbeddingProviderSpec(
        ...
        # OpenRouter proxies chat/TTS/ASR/image but has NO embeddings API. Kept
        # registered so host/model auto-detection still recognizes it, but
        # resolving it for embeddings raises early with an actionable message.
        unsupported_reason=(
            "OpenRouter has no embeddings API. Use a local Ollama model "
            "(e.g. nomic-embed-text, EMBEDDING_DIMENSION=768) or an "
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    """Resolve active LLM config with binding-substring provider matching."""
...
def _coerce_bounded_int(value: Any, *, default: int, lower: int, upper: int) -> int:
...
def _coerce_bounded_float(value: Any, *, default: float, lower: float, upper: float) -> float:
    if value is None:
        return default
    try:
...
def resolve_embedding_runtime_config(
    ...
    *,
    ...
    active_extra_headers = _to_headers((profile or {}).get("extra_headers"))
    request_timeout = _coerce_bounded_int(
        (profile or {}).get("request_timeout"),
        default=60,
        lower=5,
        upper=300,
    )
    batch_size = _coerce_bounded_int(
        (profile or {}).get("batch_size"),
        default=10,
        lower=1,
        upper=256,
    )
    batch_delay = _coerce_bounded_float(
        (profile or {}).get("batch_delay"),
        default=0.0,
        lower=0.0,
        upper=30.0,
    )
...
def resolve_search_runtime_config(
    ...
    *,
    ...
    """Resolve active web-search config with env-fallback behavior."""
    env = env_store or get_env_store()
    catalog_service = service or get_model_catalog_service()
    loaded = _load_catalog(catalog)
