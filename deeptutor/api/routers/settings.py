...
from deeptutor.services.config.response_sanitizer import (
    sanitize_catalog_for_response,
    sanitize_env_for_response,
)
...
async def update_catalog(payload: CatalogPayload):
    # A UI save makes the catalog authoritative — stamp user_managed so the
    # env overlay never reverts the saved model / api_key on the next load.
    catalog = get_model_catalog_service().save(payload.catalog, user_managed=True)
    _invalidate_runtime_caches()
    return {"catalog": sanitize_catalog_for_response(catalog)}


@router.post("/apply")
async def apply_catalog(payload: CatalogPayload | None = None):
    # With a payload this is a user-initiated save+apply (mark user_managed);
    # without one it re-applies the catalog already on disk (preserve its flag).
    if payload is not None:
        rendered = get_model_catalog_service().apply(payload.catalog, user_managed=True)
    else:
        rendered = get_model_catalog_service().apply()
    _invalidate_runtime_caches()
    return {
        "message": "Catalog applied to the active .env configuration.",
        "catalog": sanitize_catalog_for_response(get_model_catalog_service().load()),
        "env": sanitize_env_for_response(rendered),
    }
