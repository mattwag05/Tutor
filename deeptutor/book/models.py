...
class Progress(BaseModel):
    ...
    Individual quiz attempts live in the unified SQLite quiz store (read via
    ``/api/v1/quiz/attempts``), not here — Progress keeps only the derived
    ``weak_chapters``/``score`` (DeepTutor-dd1). Legacy ``progress.json`` files
    may still carry a ``quiz_attempts`` key; ``extra="ignore"`` drops it on load.
    """
