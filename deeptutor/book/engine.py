...
class BookEngine:
    ...
    async def record_quiz_attempt(
        ...
        *,
        ...
        # The unified SQLite quiz store is the single source of truth for
        # individual attempts (read via /api/v1/quiz/attempts; powers the
        # spaced-review picker, PRD §6.5). Progress now only tracks the derived
        # weak_chapters/score. Broad except: the Progress write already
        # succeeded; never break it on a unified-store failure.
        try:
            ...
            logger.exception("unified quiz store write failed for %s", book_id)
