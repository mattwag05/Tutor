...
def _book_illustrations_enabled() -> bool:
    """Opt-in raster illustrations for Books (default off)."""
    return os.getenv("ENABLE_BOOK_ILLUSTRATIONS", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
class FigureGenerator(BlockGenerator):
    ...
    async def _generate(
        ...
        # Opt-in raster illustration prompt (default off). Strictly additive: the
        # vector figure above is unchanged; the reader renders a raster only when
        # this field is present and the client illustrations flag is enabled.
        if _book_illustrations_enabled():
            desc = (analysis.description or "").strip()
            payload["illustration_prompt"] = (
                f'Educational illustration for the topic "{chapter_title}"'
                f"{focus_clause}. {desc} "
                "Clean, clear, instructive, textbook-quality; no text labels."
            ).strip()[:600]
