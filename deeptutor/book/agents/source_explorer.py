...
# Web-search grounding for Books is OPT-IN and best-effort — it never blocks
# generation. Enable with ENABLE_BOOK_WEB_SEARCH=1.
_WEB_SEARCH_MAX_QUERIES = 3


def _book_web_search_enabled() -> bool:
    return os.getenv("ENABLE_BOOK_WEB_SEARCH", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
class SourceExplorer(BaseAgent):
    ...
    async def explore(
        ...
        *,
        ...
        if _book_web_search_enabled():
            chunks.extend(await self._web_search_chunks(queries))
    ...
    async def _web_search_chunks(self, queries: list[str]) -> list[SourceChunk]:
        """Best-effort web-search grounding (``source='web'``).

        Opt-in via ``ENABLE_BOOK_WEB_SEARCH``; any failure returns ``[]`` so a
        search outage or misconfig can never block spine/page generation. The
        synchronous ``web_search`` runs in a thread to keep the event loop free.
        """
        try:
        ...
        for qi, query in enumerate(queries[:_WEB_SEARCH_MAX_QUERIES]):
            try:
                resp = await asyncio.to_thread(web_search, query)
            except Exception as exc:  # noqa: BLE001 - best-effort grounding
                logger.warning("book web_search failed for %r: %s", query, exc)
                continue
            results = (resp or {}).get("search_results") or []
            for i, raw in enumerate(results[: self.chunks_per_query]):
                ...
                out.append(
                    SourceChunk(
                        ...
                        metadata={"url": url, "title": title, "query": query[:160]},
                    )
                )
        return out
