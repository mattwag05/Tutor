...
- **Gotchas (1–48):** see "Known Gotchas" — most-cited: #6 (sidebar refactor), #12 (caddy/tailscale on Pironman), #16 (web/ i18n: 2 locales flat), #20 (`/api/v1/quiz/attempts` vs notebook upsert), #28 (stacked-PR rebase), #32 (CI excludes web/), #33 (Buffer.slice for binary Response), #34 (Serwist setup), #40 (dependabot api caps), #41 (vendored-lib devDep prune), #42 (Next 16 POST `localhost` 404), #44 (Manifest `:` separator + OpenRouter slugs), #45 (course→classroom projection must emit `Scene.actions`), #46 (TUTOR_DATA_DIR / Next.js cwd), #47 (catalog services + ProfiledService), #48 (feature flags via interface.json).
