  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6 p-6">
      ...
      <p className="text-center text-xs text-neutral-500 dark:text-neutral-500">
        Solo narration uses one voice. Conversational uses two distinct voices in a back-and-forth dialogue.
      </p>
    </div>
  );
}
    ...
    return (
      <div className="flex flex-col items-center gap-4 text-center">
        <p className="text-sm text-rose-600 dark:text-rose-400">
          {artifact.error || 'Podcast generation failed'}
        </p>
        <PrimaryButton onClick={onGenerate}>Retry</PrimaryButton>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-6 text-center">
      <p className="text-sm text-neutral-600 dark:text-neutral-400">
        {mode === 'solo'
          ? 'Generate a single-narrator audio walkthrough of this course.'
          : 'Generate a two-host dialogue podcast about this course.'}
      </p>
      <PrimaryButton onClick={onGenerate}>Generate podcast</PrimaryButton>
    </div>
  );
}
...
function Generating() {
  return (
    <div className="flex flex-col items-center gap-3 py-10 text-center">
      ...
      />
      <p className="text-sm text-neutral-600 dark:text-neutral-400">
        Synthesizing audio (may take 1–3 minutes)…
      </p>
    </div>
  );
}
